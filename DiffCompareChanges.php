        	...
        	for ($i = 0; $i <= $l; $i++ ) {
                	...
                	for ($j = 1; $j <= ($l - $i); $j++ ) {
                        	...
                        	foreach ($segments as $s) {
                                	$tmp_start = substr($s, 0, strpos($s,"for"));
                                	$tmp_length = substr($s, strpos($s,"for") + 3);
                                	if ($j >= $tmp_start && $j <= $tmp_start + $tmp_length) $stop = 1;
                        	}
        	...
        	for ($i = 0; $i <= $l; $i++ ) {
                	if (substr($oPageContent1, $i, 1 ) == ' ' || $i == $l) { // end of word or end of string
                        	...
                        	foreach ($segments as $s) {
                                	...
                                	if ($word_start >= $tmp_start && $word_end <= $tmp_end) $word_is_ok = 1;
                        	}
...
$o1 = $differences->HighlightDifferences($oPageContent1, $oPageContent2);
$o2 = $differences->HighlightDifferences($oPageContent2, $oPageContent1);
